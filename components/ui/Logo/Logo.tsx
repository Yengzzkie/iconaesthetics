...
const Logo = ({ light } : LogoProps) => {
  return (
    <div className={`${baseStyle} ${light ? lightStyle : darkStyle}`}>
      <h1 className={`${italiana.className} ${light ? lightStyle : darkStyle} text-3xl font-bold`}>
        <a href="#home">ICON ESTHETICS</a>
      </h1>
      <div className={`${light ? "bg-(--secondary)" : "bg-white"} absolute -bottom-2.5 right-3 text-sm px-0.5 tracking-wider`}>
        BEAUTY & SPA
      </div>
    </div>
  );
